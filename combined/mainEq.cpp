...
int main()
{
	...
	uniform_real_distribution<double> diskDistribution(0.0, 1.0/sqrt(3.0));
	
	double figArea = sqrt(3.0)/4.0;

	for (int ii = 0 ; ii <= numIter ; ii ++)
	{
		double factor = (2.0/sqrt(3.0))*(double(ii)/double(numIter));
		int count = 0;
		...
		for(int i = 0 ; i < numIter ; i ++)
		{
			...
			Point a(0,0);
			count += eq(diskList, answer, a, 1.0);
		}
